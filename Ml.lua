--[[
  Universal ESP + 5 Team Color + Health + Tracers + Hitbox + Aimbot + Menu tròn di chuyển + FOV Circle
...
local function getTeamColor(p)
    ...
    if not TeamIndexMap[key] then
        ...
        local idx = (count % 5) + 1 -- 5 màu team
...
local function createUI()
    ...
    title.Text = "Universal ESP - 5 Team"
...
--------------------------
-- TRACER UPDATE LOOP (Auto line logic)
--------------------------
RunService.RenderStepped:Connect(function()
    ...
    for p,ct in pairs(ESPMap) do
        if isEnemy(p) and ct.character and ct.tracer then
            hasEnemy = true
            break
        end
    end
    ...
    for p,ct in pairs(ESPMap) do
        ...
        if tracer and char and char.Parent and isEnemy(p) then
